...
def read_imagelist_3channel(filelist,size=[64,64],root=''):
    ...
    test_num=len(lines)
    fid.close()
    X=np.empty((test_num,size[0],size[1],3))
    i =0
    for line in lines:
        word=line.split('\n')
        ...
        try:
            ...
            filename=perfile+'.ppm'
            print filename
            filename=r'E:\database\LFW_test\lfwcrop_color\faces\\'+filename
            im1=cv2.imread(filename)
...
def read_imagelist_1channel(filelist,size=[64,64],root=''):
    ...
    test_num=len(lines)
    fid.close()
    X=np.empty((test_num,size[0],size[1],1))
    i =0
    for line in lines:
        word=line.split('\n')
...
#        print filename
        try:
            ...
            im1=cv2.imread(filename,0)
        im1 =im1
#        assert(im2.all()==im1.all())
#        im1=cv2.imread(filename,0)
#        if im1==None:
#            filename=r'E:\database\lfw224\\'+word[0]
#            im1=cv2.imread(filename,0)
#            im1 =cv2.resize(im1,(size[0],size[1]))
        if im1.ndim<3:
#            print 'gray:'+filename
            X[i,:,:,0]=im1[:,:]
        else:
