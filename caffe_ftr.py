...
def extract_feature(network_proto_path,
    ...
    net = caffe.Classifier(network_proto_path, network_model_path,input_scale=255)
#    net.set_phase_test()
...
#            img = caffe.io.load_image(path, color = not image_as_grey)
            ...
            print path
             ...
             word=path.split('\\')
             newpath=r'E:\database\LFW_align_man5pt_3\lfw\image\\'+word[-2]+'\\'+word[-1]
             img = caffe.io.load_image(newpath, color = not image_as_grey)
    ...
    return features
def extract_imagefeatures_to_mat(network_proto_path, network_model_path, data_mean,
                            image_path, layer_name, save_path, image_as_grey = False):

    ftr = extract_imagefeature(network_proto_path, network_model_path,image_path,
                        data_mean, layer_name, image_as_grey)
    #print ftr.shape
...
#        ftr = ftr[:,:,0,0]
    ...
    return
def extract_imagefeature(network_proto_path,
                    network_model_path,image_path,
                    data_mean, layer_name, image_as_grey = False):
    """
    Extracts features for given model and image list.
...
#    net.set_mean('data', data_mean)
    ...
    #params = OrderedDict( [(k, (v[0].data,v[1].data)) for k, v in net.params.items()])
    features_shape = (1, shp[1])
    features = np.empty(features_shape, dtype='float32', order='C')
    img_batch = []
    for cnt, path in zip(range(features_shape[0]), image_path):
...
#    print  'main'
    ...
    '''
#    main(sys.argv[1:])
#    network_proto_path=r'C:\Users\Saber\Desktop\param_operation\LightenedCNN_C_deploy_QandR.prototxt'
#    network_model_path=r'E:\windowscaffe\caffe-master\caffe-master\examples\lightcnn\QandR\model\_iter_300000.caffemodel'
#    save_path=r'E:\windowscaffe\caffe-master\caffe-master\examples\lightcnn\QandR\lab_r.mat'
#    layer_name='contact_conv'
##    network_proto_path=r'C:\Users\Saber\Desktop\param_operation\LightenedCNN_C_deploy_QandR.prototxt'
##    network_model_path=r'E:\windowscaffe\caffe-master\caffe-master\examples\lightcnn\QandR\model\_iter_300000.caffemodel'
##    save_path=r'E:\windowscaffe\caffe-master\caffe-master\examples\lightcnn\QandR\lab_1.mat'
##    layer_name='contact_conv'
#
#    data_mean=r'E:\windowscaffe\caffe-master\caffe-master\examples\lightcnn\QandR\mean.binaryproto'
#    image_dir=r'E:\database\lfw_lightcnn\cut'
#    list_file=r'E:\validation\rightlist.txt'
#
#    image_as_grey=False
#    extract_features_to_mat(network_proto_path, network_model_path, data_mean,
