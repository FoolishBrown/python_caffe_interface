...
import numpy as np
#测试准确度，1为相同0为不同，
def calculate_accuracy_pairs(distance,labels,num,threshold=[0.2,0.9],step=0.001):    
    ...
    while threshold[0] <= threshold[1] :
        ...
        thresholdlist.append(threshold[0])
        threshold[0]=threshold[0]+step
        index=accuracy.index(np.max(accuracy))
        maxth=thresholdlist[index]
        print maxth
#        print accuracy
